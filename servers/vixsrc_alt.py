# -*- coding: utf-8 -*-
# Server vixsrc per S4me — v7 "single-use token + stat-fast"
# ------------------------------------------------------------
...
#   [STAT-FAST] Kodi interroga l'URL media anche DOPO lo stop (bookmark/
#   videolibrary) con CCurlFile::Stat (HEAD, timeout 20s x2 = il blocco
#   40s alla chiusura): ora OGNI HEAD su playlist e' risposta locale in
#   millisecondi, con o senza cache.
...
def _start_proxy():
    ...
    class _Handler(http.server.BaseHTTPRequestHandler):
        ...
        def _relay(self, with_body):
            ...
            # istantanea. Kodi fa Stat (HEAD) sull'URL media DOPO lo stop
            # per il bookmark/videolibrary: 2 x timeout 20s = il blocco 40s.
            if not with_body and is_pl:
                ent = _PLCACHE.get(self.path) or _PLCACHE.get(req_path)
                try:
                    size = len(ent[1].encode('utf-8')) if ent else 0
                    self.send_response(200 if ent else 404)
                    self.send_header('Content-Type', 'application/vnd.apple.mpegurl')
            ...
            # [TOKEN-USE1-v2] master "nudo" senza token (da Kodi, post-play
            # o cache scaduta): il token del resolve e' gia' consumato ->
            # 503 subito + heal; il retry di Kodi trovera' la cache calda.
            if is_pl and 'token=' not in up_q:
                _heal_trigger()
                self.send_error(503)
                return
