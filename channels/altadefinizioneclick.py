...
import re, html, traceback, urllib.parse
...
def findvideos(item):
    ...
    # [SERVER] una sola fetch: la sessione del framework ha gia' i cookie CF
    page = support.httptools.downloadpage(item.url, cloudscraper=True).data or ''
    if not page:
        logger.error("detail page not loaded")
        return []

    # iframe vidxgo: preferiamo quello del film, i trailer solo come ultima spiaggia
    embed_url = None
    fallback_url = None
