...
def mainlist(item):
    return [
        ...
        Item(channel=item.channel, title=config.get_setting("film1", channel="filmontv"), action="now_on_tv", url="%s/film-in-tv/" % host, thumbnail=item.thumbnail),
        Item(channel=item.channel, title=config.get_setting("film3", channel="filmontv"), action="now_on_tv", url="%s/film-in-tv/oggi/sky-intrattenimento/" % host, thumbnail=item.thumbnail),
        Item(channel=item.channel, title=config.get_setting("film4", channel="filmontv"), action="now_on_tv", url="%s/film-in-tv/oggi/sky-cinema/" % host, thumbnail=item.thumbnail),
        Item(channel=item.channel, title=config.get_setting("film6", channel="filmontv"), action="now_on_tv", url="%s/film-in-tv/oggi/sky-doc-e-lifestyle/" % host, thumbnail=item.thumbnail),
        Item(channel=item.channel, title=config.get_setting("film7", channel="filmontv"), action="now_on_tv", url="%s/film-in-tv/oggi/sky-bambini/" % host, thumbnail=item.thumbnail),
        Item(channel=item.channel, title=config.get_setting("now1", channel="filmontv"), action="now_on_misc", url="%s/ora-in-onda/" % host, thumbnail=item.thumbnail),
        Item(channel=item.channel, title=config.get_setting("now3", channel="filmontv"), action="now_on_misc", url="%s/ora-in-onda/sky-intrattenimento/" % host, thumbnail=item.thumbnail),
        Item(channel=item.channel, title=config.get_setting("now4", channel="filmontv"), action="now_on_misc", url="%s/ora-in-onda/sky-cinema/" % host, thumbnail=item.thumbnail),
        Item(channel=item.channel, title=config.get_setting("now5", channel="filmontv"), action="now_on_misc", url="%s/ora-in-onda/sky-doc-e-lifestyle/" % host, thumbnail=item.thumbnail),
        Item(channel=item.channel, title=config.get_setting("now6", channel="filmontv"), action="now_on_misc", url="%s/ora-in-onda/sky-bambini/" % host, thumbnail=item.thumbnail),
        Item(channel=item.channel, title=config.get_setting("now7", channel="filmontv"), action="now_on_misc", url="%s/ora-in-onda/rsi/" % host, thumbnail=item.thumbnail),
        Item(channel=item.channel, title="Personalizza Oggi in TV", action="server_config", config="filmontv", folder=False, thumbnail=item.thumbnail)
    ]
...
def create_search_item(title, search_text, content_type, thumbnail="", year="", genre="", plot="", event_type=""):
    ...
    if use_new_search:
        new_item = Item(channel='globalsearch', action='Search', text=clean_text, title=title, thumbnail=thumbnail, fanart=thumbnail, mode='movie' if content_type == 'movie' else 'tvshow', type='movie' if content_type == 'movie' else 'tvshow', contentType=content_type, infoLabels=infoLabels, folder=False)
        ...
        new_item = Item(channel='search', action="new_search", extra=quote_fn(clean_text) + '{}' + extra_type, title=title, fulltitle=clean_text, mode='all', search_text=clean_text, url="", thumbnail=thumbnail, contentTitle=clean_text, contentYear=year if year else "", contentType=content_type, infoLabels=infoLabels, folder=True)
        ...
        try:
            ...
            for card in cards:
                try:
                    ...
                    if not parsed:
                        continue
                    films_dict[parsed['title'].lower()] = {'year': parsed['year'], 'genre': parsed['genre'], 'thumbnail': parsed['thumbnail']}
                except Exception:
                    continue
        except Exception as e:
...
def now_on_misc(item):
    ...
    for card in cards:
        try:
            ...
            skip_tmdb = any(black in genre for black in tmdb_blacklist) or ("qvc" in scrapedchannel.lower() and "replica" in scrapedtitle.lower()) or ("donnatv" in scrapedchannel.lower() and "l'argonauta" in scrapedtitle.lower()) or ("rai 1" in scrapedchannel.lower() and "l'eredità" in scrapedtitle.lower())
            formatted_title = "[B]%s[/B] - %s - %s" % (scrapedtitle, scrapedchannel, scrapedtime)
            if skip_tmdb:
                itemlist.append(Item(channel=item.channel, title=formatted_title, thumbnail=full_thumbnail, fanart=full_thumbnail, folder=False, infoLabels={'title': scrapedtitle, 'plot': "[COLOR gray][B]Tipo:[/B][/COLOR] %s" % scrapedtype}))
            else:
                ...
                if content_type == 'movie':
                    ...
                    if title_lower in films_db:
                        ...
                        genre = films_db[title_lower].get('genre', genre)
                        full_thumbnail = films_db[title_lower].get('thumbnail', full_thumbnail)
                search_item = create_search_item(title=formatted_title, search_text=scrapedtitle, content_type=content_type, thumbnail=full_thumbnail, year=year, genre=genre, event_type=scrapedtype)
...
def now_on_tv(item):
    ...
    for card in cards:
        try:
            ...
            if not parsed:
                continue
            itemlist.append(create_search_item(title="[B]%s[/B] - %s - %s" % (parsed['title'], parsed['channel'], parsed['orario']), search_text=parsed['title'], content_type='movie', thumbnail=parsed['thumbnail'], year=parsed['year'], genre=parsed['genre']))
        except Exception:
            continue
    if itemlist:
